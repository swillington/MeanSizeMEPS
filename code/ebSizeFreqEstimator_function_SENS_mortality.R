#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
# Equilibrium-based size frequency estimator function (ebSizeFreqEstimator_function_sarah) ##
#
# Version100 by Nils Krueck, 19 April 2023
# # Edited by Sarah Willington
#   Changes: function used for runs with changing mortality and fixed CVt
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
ebSizeFreqEstimator_function_sarah <- function(Linf,CV.Linf,t0,tmax,Lbins,L.classes,
 ...
 #  run.name <- ""
  ...
  Mk <- M[tmax]/k        # key life history invariant
  ...
  df <- data.frame(length=L.classes.truncated, number=N0ss.truncated.scaled, Mk.ratio=rMk, Mk.ratio.select=Mk.ratio, L.infinity=Linf, M=M[tmax], K=k, sens.run.name=sens.run.name)
  df <- df %>% 
