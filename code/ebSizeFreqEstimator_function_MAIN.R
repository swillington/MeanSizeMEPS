#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
# Equilibrium-based size frequency estimator function (ebSizeFreqEstimator_function_MAIN) ##
#
# Version100 by Nils Krueck, 19 April 2023
# Current version edited by Sarah Willington
# Changes: correct truncation, simplifying simpops code, cutting out unnecessary code and variables for the paper. Added absolute value function to simpop.SD.length to deal with negative values
#   Size calculations: mean size on truncated distribution, max and median size on full distribution. length for sdist plots scaled by max size
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
ebSizeFreqEstimator_function_sarah <- function(Linf,CV.Linf,t0,tmax,Lbins,L.classes,
 ...
 #  run.name <- ""
  ...
  df <- data.frame(length=L.classes.truncated, number=N0ss.truncated.scaled, Mk.ratio=rMk,Mk.ratio.select=Mk.ratio, L.infinity=Linf, M=M, K=k)
  df <- df %>% 
