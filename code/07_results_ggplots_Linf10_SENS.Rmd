...
alfonsino_sens10 <- alfonsino_sens %>%  
  filter(Linf==43.668)
...
alfonsino_fM_incCV10 <- alfonsino_fM_incCV %>%  
  ...
  mutate(sens.run.name = case_when(sens.run.name == "fM incCV" ~ "Sensitivity analysis run", 
            sens.run.name == "original" ~ "Main model run")) %>% 
  group_by(K, M) %>%
  arrange(sens.run.name, .by_group = TRUE) %>% 
  mutate(diff_mean=change.mean.percent-lag(change.mean.percent)) %>% 
  ungroup()
...
plot_path <- file.path(project_dir, "03_results",
                             "main_plots", "sens plots")


ggsave("alfonsino_change_mean_SENS_Linf10_fulldist.png",
       plot = panel, 
       path = plot_path, 
...
```{r}
test <- rbind(alfonsino_cM_decCV10, alfonsino_cM_incCV10, alfonsino_cM_fCV10)

testpos <- test %>% filter(change.mean.percent>=0) %>% 
  group_by(sens.run.name) %>% 
  summarise(av_change=mean(change.mean.percent))

testneg <- test %>% filter(change.mean.percent<=0) %>% 
  group_by(sens.run.name) %>% 
  summarise(av_change=mean(change.mean.percent))
...
swordfish_sens10 <- swordfish_sens %>%  
  filter(Linf==164.844)
...
ggsave("swordfish_change_mean_SENS_Linf10_fulldist.png",
       plot = panel, 
       path = plot_path, 
       width = 19, height = 33, units = "cm")
```
...
moonwrasse_sens10 <- moonwrasse_sens %>%  
  filter(Linf==12.69)
...
ggsave("moonwrasse_change_mean_SENS_Linf10_fulldist.png",
       plot = panel, 
       path = plot_path, 
       width = 19, height = 33, units = "cm")
```
...
ggsave("alfonsino_change_mean_SENS_Linf10_25dist.png",
       plot = panel, 
       path = plot_path, 
       width = 19, height = 33, units = "cm")
```
...
swordfish_sens1 <- ggplot(data = swordfish_cM_decCV10, aes(x = Mk.ratio, y = change.mean.percent, shape=sens.run.name, colour=change.max.percent)) +
  ...
  theme(strip.text = element_text(size = 10, face = "bold", vjust = 0.8),
        ...
        plot.margin = margin(t=25, r=25, l=25, b=25))+
  guides(color = guide_colorbar(barheight = 10))+
  geom_boxplot(data = mk_swordfish, aes(x = standardised.M.K, y= -50),width = 3, inherit.aes = FALSE) + 
  geom_rug(data = mk_swordfish, aes(x = standardised.M.K, y = -22), sides = "b", inherit.aes = FALSE, linewidth=0.5) 
swordfish_sens1
...
ggsave("swordfish_change_mean_SENS_Linf10_25dist.png",
       plot = panel, 
       path = plot_path, 
       width = 19, height = 33, units = "cm")
```
...
ggsave("moonwrasse_change_mean_SENS_Linf10_25dist.png",
       plot = panel, 
       path = plot_path, 
       width = 19, height = 33, units = "cm")
```
